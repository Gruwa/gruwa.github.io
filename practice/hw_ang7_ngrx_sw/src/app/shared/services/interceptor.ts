...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['token'])
      ...
      request = request.clone({
        setHeaders: {
          ...
          groupID: this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['group']).id, // TODO - change description on id
          id: this.route.snapshot.children[0].children[0].params['id']
        }
      });
    } else if (this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['token'])
      && this.route.snapshot.children[0].params['group']) {
      request = request.clone({
        setHeaders: {
          Authorization: 'Bearer ' + this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['token']),
          'Content-Type': 'application/json',
          groupID: this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['group']).id
        }
      });
    } else if (this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['token'])
      && this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['group'])) {
      request = request.clone({
        setHeaders: {
          Authorization: 'Bearer ' + this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['token']),
          'Content-Type': 'application/json',
      ...
      catchError((err: any) => {
        if (err instanceof HttpErrorResponse) {
          ...
          switch ((<HttpErrorResponse>err).status) {
            case 0:
              return this.httpErrorResponseService.handle0Error(err);
            case 400:
