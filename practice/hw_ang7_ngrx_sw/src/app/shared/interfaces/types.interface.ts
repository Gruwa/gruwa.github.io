/**
 * Export type for tab
 */

export type ITabTypesShifts = 'upcoming' | 'my requests' | 'available';

/**
 * Export type for tab of availability
 */

export type ITabTypesAvailability = 'time off' | 'volunteer';

/**
 * Export type for footer requests
 */

export type IFooterRequest = 'request pickup' | 'cancel request pickup' | 'request drop' | 'cancel drop request' | 'cancel request';

/**
 * Export type for status
 */

export type IStatus = 'scheduled' | 'drop request' | 'pickup request';

/**
 * Export type for RequestStatusAvailability
 */

export type IRequestStatusAvailability = 'A' | 'R' | 'N';
