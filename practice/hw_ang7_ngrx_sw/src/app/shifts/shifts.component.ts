...
export class ShiftsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    for (const i in this.dataService.FLOW) {
      if (!this.flowService[`${this.dataService.FLOW[i]}`]) {
        this.httpService.getShifts(<ITabTypesShifts>i);
      }

    }

    if (this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['tab']) !== null) {
      this.tabActive = this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['tab']);
    } else {
      this.localStorage.store('tab', this.tabActive);
    }

    this.tabIndex = this.dataService.indexTABS.indexOf(this.localStorage.retrieve(this.dataService.LOCAL_STORAGE['tab']));
  }
