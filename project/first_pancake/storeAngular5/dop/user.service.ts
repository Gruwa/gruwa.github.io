...
import {StorageService} from 'app/services/storage-service/storage.service';
...
export class UserService {
    ...
    getVendorId(): string {
        if (!this.vendorParams) {
            this.vendor_id = this.storage.getItem('vendor_id');
    ...
    getUsers( search: string = '',
        ...
        if ( tabs[tab] ) {
            return this.http.get(this.setUrl() + `/${tabs[tab]}/`, {
                headers: this.getHeaders(),
                params: query_params
            }).map(
    ...
    getCheckEmail(email: any) {
        return this.http.get(this.setUrl() + '/email/' + email + '/', {
            headers: this.getHeaders()
